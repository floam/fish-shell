...
void history_tests_t::test_history_races_pound_on_history() {
    ...
    const wcstring_list_t history_lines = generate_history_lines(getpid());
    for (size_t idx = 0; idx < history_lines.size(); idx++) {
        const wcstring &line = history_lines.at(idx);
        hist->add(line);
        hist->save();
    }
    delete hist;
...
#define RACE_COUNT 10
    ...
    for (hist_idx = 1;; hist_idx++) {
        ...
        // The item must be present in one of our 'expected_lines' arrays. If it is present, then every item

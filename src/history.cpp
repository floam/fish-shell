...
static size_t offset_of_next_item_fish_2_0(const char *begin, size_t mmap_length,
    ...
    while (cursor < mmap_length) {
        ...
        const char *next_newline = (const char *)memchr(line_start, '\n', mmap_length - cursor);
        if (next_newline == NULL) break;

        // Advance the cursor past this line. +1 is for the newline.
        cursor = next_newline - begin + 1;
        ...
        if (next_newline - line_start < 3) continue;
        ...
        while (next_newline - line_start > double_cmd_len &&
        ...
        if (next_newline - line_start >= cmd_when_len && !memcmp(line_start, cmd_when, cmd_when_len))
            continue;
